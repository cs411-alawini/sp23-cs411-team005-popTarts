...
@products_bp.route('/products/filtered', methods=['POST'])
def get_filtered(): 
    page = request.args.get('page', default=1, type=int)
    products_per_page = request.args.get('products_per_page', default=20, type=int)
    start_index = (page - 1) * products_per_page
    ...
    cur.execute("SELECT COUNT(*) FROM Products")
    total_products = cur.fetchone()[0]
    cur.execute("SELECT p.productId, p.name, p.imageLink, i.price, AVG(r.rating) FROM Products p NATURAL JOIN Inventory i NATURAL JOIN Reviews r WHERE i.price <= (SELECT AVG(price) FROM Inventory i) GROUP BY p.productId ORDER BY p.productId LIMIT %s,%s", (start_index, products_per_page))
    products = cur.fetchall()
    current_app.logger.info(products)
    products = list(map(translate_game_info,products))
    return render_template('products.html',  products=products, total_products=total_products, 
